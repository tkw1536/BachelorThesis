...
The theories and their relations can be represented with the help of a graph. This graph is then called theory graph. In the following example, we represent imports as solid edges and views as dashed edges. \ednote{Have an example of a small theory graph here with help from @miancu. }
...
Finally, there is the problem of choosing a suitable corpus and implementing a crawler which will find and mark up quantity expressions. The latter will be taken on by ???\ednote{Who is doing unit finding?} in a seperate effort\ednote{Reference here}. The task of finding a suitable corpus is postponed for now. In case there is sufficient time a corpus can be marked up manually and crawled afterwards.
