...
\ednote{Should I write a final paragraph in this section. }
...
Hence it will not be sufficient to use exact translations. Thus we should search for a range of values instead. This has recently been implemented as an extension to MathWebSearch \ednote{has been implemented by Radu, Quote needed}. It remains to be seen how exactly these ranges should be used.
...
Finally, there is the problem of choosing a suitable corpus and implementing a crawler which will find and mark up quantity expressions. The latter will be taken on by ???\ednote{Who is doing unit finding?} in a seperate effort\ednote{Quote this effort here if it exists}. The task of finding a suitable corpus is postponed for now. In case there is sufficient time a ycorpus can be marked up manually and crawled afterwards.

\ednote{Maybe I should write a final paragraph. }
