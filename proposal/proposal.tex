...
This proposal is organised as follows: In section \ref{sec:mws} we shortly describe the existing MathWebSearch system. We continue in section \ref{sec:timings} with a list of work packages and a schedule and then conclude in section \ref{sec:system} with describing the approach we will use with the work packages\ednote{Reforumulate this}.
...
This results in the following timeline \ednote{Replace week numbers with actual dates}:
\begin{itemize}
...
\ednote{Is the section title correct?} \ednote{Remove subsections?}
