...
So far the\ednote{Which one?} transformation system has been used by MWS exclusively for mathmatical forumlae. In this paper we propose an extension for quantity expressions which should prove useful for physicists.
...
As a simple example, the theory of semigroups can be written down as following\ednote{Define ded?}:
...
Thus a theorem in one theory can be translated along views as well as along imports. For example, given a theorem in the theory of groups, we automatically get a theorem in the theory of commutative groups. \ednote{They contain mappings as in Figure graph1}
